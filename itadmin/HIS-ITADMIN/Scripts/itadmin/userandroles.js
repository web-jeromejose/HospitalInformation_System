var c = new Common();

var tblAllUser;
var tblallRoles;
var RoleID;

$(function () {

    
   
    LoadUsers();
    LoadRoles();
    var Tbldashboard;
    var TbldashboardId = '#DTAssignRole';
    
    var TblDashboardPresentPrice;
    var TblDashboardPresentPriceId = '#DTviewVatPresentPrice';
    var TblDashboardPresentPriceDataRow;
    var ennDashboardPresentPrice = { Btn: 0, del: 1, ctr: 2, ID: 3, Name: 4, Edited: 5, Deleted: 6 };


    AssignRolesNS.init();
});

function LoadUserRoles(userid) {

    ajaxWrapper.Get($("#url").data("getrolesbyuserid"), { userid: userid }, function (x, e) {
        console.log(x);

        $('#preloader').hide();
        $("#tbluserrolelist").DataTable({
            destroy: true,
            paging: true,
            searching: true,
            ordering: true,
            info: false,
            data: x,
            bAutoWidth: false,
            columns: [
                 { data: "Name" },
                 { data: "Description" },

            ],
            fnCreatedRow: function (nRow, aData, iDataIndex) {
                var $nRow = $(nRow); // cache the row wrapped up in jQuery
                //$nRow.addClass("row_red");
                //if (aData["HasAccess"] == "Yes") {
                $nRow.addClass("row_red");
                //}
                //  $('td:eq(0)', nRow).html("<button class='btn btn-sm btn-success'>Add</button>");
                //  $('td:eq(1)', nRow).html("<button class='btn btn-sm btn-danger'>Remove</button>");
            }
        });
        $('#tbluserrolelist_paginate').css('margin-left', '-100px');

    });
}
function LoadRoles() {
    ajaxWrapper.Get($("#url").data("getallrolelist"), null, function (x, e) {
        tblallRoles = $("#tblallRoles").DataTable({
            destroy: true,
            paging: true,
            searching: true,
            ordering: true,
            info: false,
            data: x,
            bAutoWidth: false,
            columns: [
                // { data: "Id", className: "_add" },
               //  { data: "Id", className: "_remove" },
                 { data: "Id" },
                 { data: "Name" },
                { data: "Description" }
            ],
            fnCreatedRow: function (nRow, aData, iDataIndex) {
                var $nRow = $(nRow); // cache the row wrapped up in jQuery
                //$nRow.addClass("row_red");
                //if (aData["HasAccess"] == "Yes") {
                $nRow.addClass("row_green");
                //}
                //  $('td:eq(0)', nRow).html("<button class='btn btn-sm btn-success'>Add</button>");
                //  $('td:eq(1)', nRow).html("<button class='btn btn-sm btn-danger'>Remove</button>");
            }
        });
    });
}
function LoadUsers() {
    ajaxWrapper.Get($("#url").data("getusersdatatable"), null, function (x, e) {


        tblAllUser = $("#tblAllUser").DataTable({
            destroy: true,
            paging: true,
            searching: true,
            ordering: true,
            info: false,
            data: x,
            bAutoWidth: false,
            columns: [
                 { data: "ID" },
                 { data: "EmployeeID" },
                { data: "Name" }
            ],
            fnCreatedRow: function (nRow, aData, iDataIndex) {
                var $nRow = $(nRow); // cache the row wrapped up in jQuery
                //$nRow.addClass("row_red");
                //if (aData["HasAccess"] == "Yes") {
                $nRow.addClass("row_green");
                //}
                //  $('td:eq(0)', nRow).html("<button class='btn btn-sm btn-success'>Add</button>");
                //  $('td:eq(1)', nRow).html("<button class='btn btn-sm btn-danger'>Remove</button>");
            }
        });

    });


}


function getModulebyRole(roleid) {
    ajaxWrapper.Get($("#url").data("getmodulebyroleid"), { id: roleid }, function (xx, e) {
        getmodulelist(xx);
    });


}
function getmodulelist(x) {
    tblmoduleslist = $("#tblmoduleslist").DataTable({
        cache: false,
        destroy: true,
        data: x,
        paging: false,
        ordering: false,
        searching: false,
        info: false,
        //scrollX: true,
        processing: false,
        autoWidth: false,
        dom: 'Rlfrtip',
        scrollCollapse: false,
        pageLength: 150,
        lengthChange: false,
        columns: [
             { data: "text" },
             { data: "id", className: "_remove" }
        ],
        fnCreatedRow: function (nRow, aData, iDataIndex) {
            //$('td:eq(1)', nRow).html("<button>Remove</button>");
            $('td:eq(1)', nRow).html('<button type="button" class="btn btn-xs red">Remove</button>');
            var $nRow = $(nRow);
            $nRow.addClass("row_green");
        }
    });

}





var AssignRolesNS = function () {

    var assignroles = function () {
        function loadDT() {
            ajaxWrapper.Get($("#url").data("getdetailsbyemp"), { userid: $('#sel2_AssignRoles_Employee').val() }, function (x, e) {
              BindDashboard(x.list);
               
            });
        }
        function BindDashboard(data) {
            Tbldashboard = $(TbldashboardId).DataTable({

                cache: false,
                destroy: true,
                data: data,
                paging: true,
                ordering: true,
                searching: true,
                info: true,
                pagingType: "full_numbers",
                autoWidth: false,
                scrollX: "100%",
                "dom": ' B <"divtoolbar"i>rt<"bottom"flip><"clear">',
                scrollCollapse: false,
                columns: [
                    //searchable field
                     { data: "RoleId", visible: false, searchable: true, width: "0%" },
                     //{ data: "TableName", visible: false, searchable: true, width: "0%" },
                     //{ data: "DeptName", visible: false, searchable: true, width: "0%" },
                     //{ data: "CostPrice", visible: false, searchable: true, width: "0%" },
                     // //VIEWING
                   //{ data: "Code", visible: false, searchable: true, width: "0%" }, {
                   //    data: "", title: 'ITEM CODE', className: ' ', visible: true, searchable: false, width: "10%", sortable: true,
                   //    "render": function (data, type, full, meta) {
                   //        var dataname = "Code_" + full.TableName + "_" + full.Id;
                   //        var Code = full.Code;

                   //        var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                   //                ' <input type="text" style="font-size: 10px;" class="form-control" id="' + dataname + '"   value="' + Code + '" >' +
                   //                '<span class="input-group-btn">' +
                   //                ' <button type="button" data-itemid="' + full.Id + '" data-table="' + full.TableName + '" data-input-id="' + dataname + '"  class="CLcode btn btn-info btn-flat" style="width: 20px !important;padding: 0px; "> <i class="fa fa-fw fa-save"></i> </button>' +
                   //                ' </span>' +
                   //                ' </div>';

                   //        return html;
                   //    }

                   //},
                   // {
                   //     data: "", title: 'Category', className: ' ', visible: true, searchable: false, width: "1%", sortable: true,
                   //     "render": function (data, type, full, meta) {
                   //         var dataname = full.TableName;
                   //         var html = '<code style="font-size: 10px;" >' + dataname + '</code>';
                   //         return html;
                   //     }

                   // },

                   //{
                   //    data: "", title: 'Department', className: ' ', visible: true, searchable: false, width: "1.5%", sortable: true,
                   //    "render": function (data, type, full, meta) {
                   //        var dataname = full.DeptName;
                   //        var html = '<code style="font-size: 10px;" >' + dataname + '</code>';
                   //        return html;
                   //    }

                   //},

                   // {
                   //     data: "", title: 'ITEM Name', className: ' ', visible: true, searchable: false, width: "20%", sortable: true,
                   //     "render": function (data, type, full, meta) {
                   //         var dataname = "Name_" + full.TableName + "_" + full.Id;
                   //         var Name = full.Name;

                   //         var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                   //                     ' <textarea style="font-size: 10px;" class="form-control "  id="' + dataname + '"  >' + Name + '</textarea> ' +
                   //                     '<span class="input-group-btn">' +
                   //                   ' <button type="button"  data-itemid="' + full.Id + '" data-table="' + full.TableName + '"  data-input-id="' + dataname + '"  class="CLitemname btn btn-info btn-flat" style="width: 20px !important;padding: 0px; "> <i class="fa fa-fw fa-save"></i> </button>' +

                   //                     ' </span>' +
                   //                     ' </div>';

                   //         return html;
                   //     }

                   // },
                   // {
                   //     data: "", title: 'Cost Price', className: ' ', visible: true, searchable: false, width: "10%", sortable: true,
                   //     "render": function (data, type, full, meta) {
                   //         var dataname = "CostPrice_" + full.TableName + "_" + full.Id;
                   //         var CostPrice = full.CostPrice;

                   //         var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                   //                     ' <input style="font-size: 10px;" type="number" class="form-control" id="' + dataname + '"   value="' + CostPrice + '" >' +
                   //                     '<span class="input-group-btn">' +
                   //                     ' <button type="button"  data-itemid="' + full.Id + '" data-table="' + full.TableName + '"  data-input-id="' + dataname + '"  class="CLcostprice btn btn-info btn-flat" style="width: 20px !important;padding: 0px; "> <i class="fa fa-fw fa-save"></i> </button>' +
                   //                      ' </span>' +
                   //                     ' </div>';

                   //         return html;
                   //     }

                   // },

                ],
                buttons: [
                          {
                              extend: 'pdfHtml5', orientation: 'landscape', pageSize: 'A4', filename: 'All_procedure_Codes_pdf', footer: true, message: 'messagef'
                            , messageBottom: ' ', messageTop: ' '
                            , title: 'All_procedure Codes   '
                            , text: '<button type="button"  class="btn-margin-left btn btn-warning btn-xs pull-left"> <i class="glyphicon glyphicon-print"></i> PDF </button><br><br>'
                            , exportOptions: { columns: ':visible' }
                            , customize: function (doc) {
                                //get date
                                var date = new Date();
                                Date.prototype.yyyymmdd = function () {
                                    var yyyy = this.getFullYear().toString();
                                    var mm = (this.getMonth() + 1).toString(); // getMonth() is zero-based
                                    var dd = this.getDate().toString();
                                    return yyyy + "/" + (mm[1] ? mm : "0" + mm[0]) + "/" + (dd[1] ? dd : "0" + dd[0]); // padding
                                };

                                var cols = [];
                                cols[0] = { text: date.yyyymmdd(), alignment: 'left', margin: [20] };
                                cols[1] = { text: ' ', alignment: 'right', margin: [0, 0, 20] };
                                var objFooter = {};
                                objFooter['columns'] = cols;
                                doc['footer'] = objFooter;
                                doc.defaultStyle.fontSize = 8; //2,3,4,etc 
                                doc.styles.tableHeader.fontSize = 9; //2, 3, 4, etc 

                                //doc.content.splice(1, 0,
                                // {
                                //     margin: [0, 0, 0, 12],
                                //     alignment: 'center',
                                //     image: 'data:image/png;base64,'
                                // }
                                //);
                            }
                          } //other button
                          , {
                              extend: 'excelHtml5', orientation: 'landscape', pageSize: 'A4', filename: 'All_procedure_Codes_excel', footer: true, message: 'messagef'
                            , messageBottom: ' ', messageTop: ' '
                            , title: 'All_procedure Codes '
                            , text: '<button type="button"  class="btn-margin-left btn btn-info btn-xs pull-left"> <i class="glyphicon glyphicon-print"></i> Excel </button><br><br>'
                            , exportOptions: { columns: ':visible' }
                          },


                ],
                fnRowCallback: ns.rowCallBack()

            });

            var btns = '';
            var toolbar = btns.concat(
                        ' <div style="background-color: skyblue; ">  '
                        , ' <div class="row has-warning">'
                        , ' <div class="col-xs-3">'
                        , '    <input type="text" class="form-control"   id="SearchCode" placeholder="Search Code here...">'
                        , '  </div>'
                        , '  <div class="col-xs-3">'
                        , '      <input type="text" class="form-control"   id="SearchCategory" placeholder="Search Category here..." >'
                        , '  </div>'
                        , '  <div class="col-xs-3">'
                        , '      <input type="text" class="form-control" id="SearchDepartment" placeholder="Search Department here...">'
                        , '  </div>'
                           , '  <div class="col-xs-3">'
                        , '      <input type="number" class="form-control" id="SearchCostPrice" >'
                        , '  </div>'
                        , ' </div>'
                        , ' </div>'
                 );
            $("div.divtoolbar").html(toolbar);
            $('#SearchCode').on('keyup', function () {
                Tbldashboard
                    .columns(0)
                    .search(this.value)
                    .draw();
            });
            $('#SearchCategory').on('keyup', function () {
                Tbldashboard
                    .columns(1)
                    .search(this.value)
                    .draw();
            });
            $('#SearchDepartment').on('keyup', function () {
                Tbldashboard
                    .columns(2)
                    .search(this.value)
                    .draw();
            });
            $('#SearchCostPrice').on('keyup', function () {
                Tbldashboard
                    .columns(3)
                    .search(this.value)
                    .draw();
            });



            $(TbldashboardId + ' tbody').on('click', '.CLcode', function (e) {
                var datainputid = '#' + $(this).data("input-id");
                var value = $(datainputid).val();
                var itemid = $(this).data("itemid");
                var table = $(this).data("table");

                console.log(value);
                console.log(itemid);
                console.log(table);
                ns.saveInputText(itemid, value, table, "code")
                return false;
            });
            $(TbldashboardId + ' tbody').on('click', '.CLitemname', function (e) {
                var datainputid = '#' + $(this).data("input-id");
                var value = $(datainputid).val();
                var itemid = $(this).data("itemid");
                var table = $(this).data("table");

                console.log(value);
                console.log(itemid);
                console.log(table);
                ns.saveInputText(itemid, value, table, "itemname");
                return false;
            });
            $(TbldashboardId + ' tbody').on('click', '.CLcostprice', function (e) {
                var datainputid = '#' + $(this).data("input-id");
                var value = $(datainputid).val();
                var itemid = $(this).data("itemid");
                var table = $(this).data("table");

                console.log(value);
                console.log(itemid);
                console.log(table);
                ns.saveInputText(itemid, value, table, "costprice");
                return false;
            });
        }
        function rowCallBack() {
            var rc = function (nRow, aData) {
                var $nRow = $(nRow);
                //if (aData["HasAccess"] === 1) {
                //    //$('td:eq(3)', nRow).html("Yes");
                //  $nRow.css("background-color", "rgb(71, 134, 171)");
                //    $('#checkFunctionConfigRole', nRow).prop('checked', true);
                //}
            };
            return rc;
        }
        function saveInputText(id, value, table, type) {
            if (value == "") {
                c.MessageBox("Error...", "Please input a value ", null);
                return false;
            }

            var entry = {
                id: id
                 , value: value
                 , table: table
                 , type: type

            };

            console.log('entry');
            console.log(entry);
            $.ajax({
                url: $('#url').data("save"),
                data: JSON.stringify(entry),
                type: 'post',
                cache: false,
                contentType: "application/json; charset=utf-8",
                beforeSend: function () {

                },
                success: function (data) {

                    var OkFunc = function () {

                    };
                    c.MessageBox(data.Title, "Procedure / Item has been updated.", OkFunc);
                },
                error: function (xhr, desc, err) {
                    var errMsg = "Error in posting the data." + "<br>" + err + "<br>" + desc;
                    c.MessageBox("Error...", errMsg, null);
                }
            });

        }


        /*******************************************************************************************/
        function onclickInit() {

            $('#btn_AssignRoles_Search').click(function (e) {
                e.preventDefault();
                loadDT();
            });

            $(document).on("click", "#gridlistRoles td", function () {
                var d = tblgridlistRoles.row($(this).parents('tr')).data();

                $("#modal-RolesupdateDataTable").modal({ "keyboard": true });

                $('#txtlistDTRolename').val(d.Name);
                $('#txtlistDTRoleID').val(d.RoleId);
                $('#txtlistDTRoleDesc').val(d.Description);

                $('#btnrolenameupdatedelete').show();
                $('#btnrolenameupdate').text('Modify');

            });

            $('#BtnSearchAssignRole').click(function () {
                AssignRolesSearch();
            });

            $('#BtnAssignRoleSave').click(function () {
                var YesFunc = function () {
                    AssignRoleSaveSave();
                };
                c.MessageBoxConfirm("Save User/s?", "Are you sure you want to save the Entry?", YesFunc, null);

            });

            $('#BtnSearchAssignModule').click(function () {
                AssignRoleModuleSearch();

            });


            /************ROLES-MODULES*****/
            $('#divRolesModules').click(function (e) {
                e.preventDefault();
            });
            $(document).on("click", "#tblmoduleslist td", function () {
                var d = tblmoduleslist.row($(this).parents('tr')).data();
                var mod = d.id;
                $("#url").data("rolemoduleid", mod);
                if ($(this).hasClass('_remove')) {
                    ajaxWrapper.Get($("#url").data("removemodulebyrole"), { "roleid": $("#txtroleModule").val(), "moduleid": mod }, function (x, e) {
                        NotifySuccess(x.Message, "Module");
                        getmodulelist([]);
                        getmoduleFeature([]);
                        getModulebyRole($('#txtroleModule').val());
                    });
                }
                else {
                    LoadModuleFeature(mod);
                }
            });
            $(document).on("click", "#tblmenulist td", function () {
                if (typeof $(this).closest("tr").find('td:eq(0)').html() != 'undefined') {

                    var d = tblmenulist.row($(this).parents('tr')).data();

                    entry = {};
                    entry.roleid = $("#txtroleModule").val();
                    entry.moduleid = $("#url").data("rolemoduleid");
                    entry.featid = d.FeatureID;
                    entry.deleted = 0;


                    if ($(this).hasClass('_add')) {
                        $(this).closest("tr").removeClass("row_green");
                        $(this).closest("tr").removeClass("row_red");
                        ajaxWrapper.Get($("#url").data("updaterolefeature"), entry, function (x, e) {
                            NotifySuccess(x.Message, "Module-Feature");
                        });
                        $(this).closest("tr").addClass("row_green");
                        $(this).closest("tr").find('td:eq(3)').html('<button type="button" class="btn btn-xs blue">Yes</button>');
                    }
                    if ($(this).hasClass('_remove')) {
                        entry.deleted = 1;
                        $(this).closest("tr").removeClass("row_green");
                        $(this).closest("tr").removeClass("row_red");
                        ajaxWrapper.Get($("#url").data("updaterolefeature"), entry, function (x, e) {
                            NotifySuccess(x.Message, "Module-Feature");
                        });
                        $(this).closest("tr").addClass("row_red");
                        $(this).closest("tr").find('td:eq(3)').html('<button type="button" class="btn btn-xs red">No</button>');
                    }
                    if ($(this).hasClass('_functions')) {
                        // LoadUserFunctions(d.FeatureID);

                        //  $("#modal-func").modal({ "keyboard": true });
                        //  $("#url").data("funcfeatadd", d.FeatureID);
                    }

                }
            });
        }
        function initSelect() {

 


            $(document).on("click", "#tblAllUser td", function () {
                if (typeof $(this).closest("tr").find('td:eq(0)').html() != 'undefined') {
                    $('#preloader').show();
                    var d = tblAllUser.row($(this).parents('tr')).data();
                    console.log(d);
                    $('#txtmodalfullname').val(d.Name);
                    LoadUserRoles(d.ID);
                    $("#modal-userroles").modal({ "keyboard": true });
                }
            });

            $(document).on("click", "#tblallRoles td", function () {
                if (typeof $(this).closest("tr").find('td:eq(0)').html() != 'undefined') {
                    $('#preloader').show();
                    var d = tblallRoles.row($(this).parents('tr')).data();
                    console.log(d);
                    RoleID = d.Id;
                    $('#txtRoleName').val(d.Name);
                    $('#txtRoleDesc').val(d.Description);
                    $("#modal-loadroles").modal({ "keyboard": true });
                    $('#preloader').hide();
                }
            });

            $(document).on("click", "#btnSaveRoleNameAndDesc", function () {


                var YesFunc = function () {
                    $('#preloader').show();

                    ajaxWrapper.Get($("#url").data("saveroledesc"), { Id: RoleID, RoleName: $("#txtRoleName").val(), Desc: $("#txtRoleDesc").val() }, function (x, e) {
                        NotifySuccess(x, "Update Role Details");
                        LoadRoles();
                        $("#modal-loadroles").modal('hide');
                        $('#preloader').hide();
                    });
                };
                c.MessageBoxConfirm("Update Role Details?", "Are you sure you want to update this role?", YesFunc, null);

            });

            ajaxWrapper.Get($("#url").data("getroles"), {}, function (xx, e) {
                Sel2Client($("#txtroles"), xx, function (x) {
                    console.log(x);
                });

                Sel2Client($("#txtroleModule"), xx, function (x) {
                    getModulebyRole(x.id);
                })


            });

            ajaxWrapper.Get($("#url").data("getstation"), {}, function (xx, e) {
                Sel2Client($("#txtstation"), xx, function (x) {
                    console.log(x);
                });
            });

            ajaxWrapper.Get($("#url").data("getallemployeelist"), {}, function (xx, e) {
                Sel2Client($("#sel2_AssignRoles_Employee"), xx, function (x) {
                    console.log(x);
                });
            });



        }
 //*************************************INITIALIZE
   
        onclickInit();
        initSelect();

    }
    return {

        init: function () {
            console.log('init');
            assignroles();
        }

    }
}();