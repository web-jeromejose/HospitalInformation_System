...
namespace DataLayer
{
    public class HISSecurity
    {
        ...
        public List<HISSecurityModel> SecuritryFeatureCS()
        {
            try
            {
                ...
                return DB.ExecuteSPAndReturnDataTable("HISGLOBAL.EMPLOYEE_FEATURE").DataTableToList<HISSecurityModel>();
            }
            catch (Exception ex)
            {
        ...
        */
        public bool IsModuleAuthorized(string OID, string MOID)
        {
            try
            {
                ...
                HISModSecurity rs = DB.ExecuteSPAndReturnDataTable("HISGLOBAL.EMPLOYEE_MODULE").DataTableToList<HISModSecurity>().FirstOrDefault();
        ...
        public bool IsFeatureAuthorized(string FID, string OID, string MOID,string FunctionID)
        {
            try
            {
                DB.param = new SqlParameter[]{
                ...
                HISFeatSecurityItadmin rs = DB.ExecuteSPAndReturnDataTable("ITADMIN.EMPLOYEE_FEATURE_2017").DataTableToList<HISFeatSecurityItadmin>().FirstOrDefault();

                if (FunctionID == "0")
                {
                    if (rs.featAccess == 0) { return false; }
                    else { return true; }
                }
                else {
                    if (rs.funcAccess == 0) { return false; }
                    else { return true; }
                }
    ...
    public class HISFeatSecurityItadmin
    {
        public string ModuleID { get; set; }
        public string UserID { get; set; }
        public string FeatureID { get; set; }
        public int featAccess { get; set; }
        public int funcAccess { get; set; }
    }
