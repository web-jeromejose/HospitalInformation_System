...
$(document).on("keypress", "#txtBagNumber", function (e) {
    if (e.which == 13) {
        GenerateReport();
    }
});
...
function GenerateReport() {
    ...
    var $report = $("#myReport");
    var UserId = "";
    var bagnumber = c.GetValue('#txtBagNumber');

    var dlink = hissystem.appsserver() + hissystem.appsname() + "/Areas/BloodBank/Reports/RptBagHistory/RptBagHistory.aspx";
    var repsrc = dlink + "?UserId=" + UserId + "&bagnumber=" + bagnumber;
...
function InitSelect2() {
    $('#select2BloodComponent').select2({
        ...
        ajax: {
            quietMillis: 150,
            url: baseURL + 'Select2RptComponent',
            dataType: 'jsonp',
            cache: false,
...
function DefaultValues() {
    ...
    c.SetSelect2('#select2BloodComponent', -1, '< ALL COMPONENT >');
