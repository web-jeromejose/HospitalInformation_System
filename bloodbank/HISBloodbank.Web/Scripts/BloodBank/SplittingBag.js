...
var TblGridSplittedBags;
var TblGridSplittedBagsId = "#gridSplittedBags";
var TblGridSplittedBagsDataRow;
...
function RedrawGrid() {
    ...
    if (TblGridSplittedBags !== undefined) TblGridSplittedBags.columns.adjust().draw();
...
function Validated() {
    ...
    req = c.IsEmptyById('#txtSplitNo');
    if (req) {
        c.MessageBoxErr('Required...', 'Split No is required.');
        $('#txtSplitNo').focus();
        return false;
    }
    if ($(TblGridSplittedBagsId).DataTable().rows().nodes().length==0) {
        c.MessageBoxErr('Required...', 'list is required.');
        return false;
    }
...
function ShowSplittedBagsColumns() {
    var cols = [
    { targets: [0], data: "ctr", title: 'SL.No', className: '', visible: true, searchable: true, width: "0%" },
    { targets: [1], data: "UnitNumber", title: 'Unit Number', className: '', visible: true, searchable: false, width: "0%" },
    { targets: [2], data: "Quantity", title: 'Quantity', className: 'ClassTxtIQty', visible: true, searchable: true, width: "0%" }
    ];
    return cols;
}
function ShowSplittedBagsRowCallBack() {
    var rc = function (nRow, aData) {
        //var value = aData['Name'];
        //var $nRow = $(nRow);
        //if (value === '') {
...
function BindSplittedBags(data) {
    
    TblGridSplittedBags = $(TblGridSplittedBagsId).DataTable({
        cache: false,
        destroy: true,
        data: data,
        paging: false,
        ...
        lengthChange: true,
        iDisplayLength: 15,
        fnRowCallback: ShowSplittedBagsRowCallBack(),
        columns: ShowSplittedBagsColumns()

    });

    InitSplittedBags();
}
function SetupSplittedBags() {
    $.editable.addInputType('txtIQty', {
        element: function (settings, original) {

            var input = $('<input id="txtIQty" style="width:100%; height:30px; margin:0px, padding:0px;" type="text" class="form-control">');
            $(this).append(input);
...
function InitSplittedBags() {

    // ------------------------------------------------------------------------------------------------------------------------------------------------------------
    $('.ClassTxtIQty', TblGridSplittedBags.rows().nodes()).editable(function (sVal, settings) {
        var cell = TblGridSplittedBags.cell($(this).closest('td')).index();
        TblGridSplittedBags.cell(cell.row, cell.column).data(sVal);
        CalculateQty();
        return sVal;
    },
    {
        "type": 'txtIQty', "style": 'display: inline;', "onblur": 'submit', "onreset": function () { setTimeout(function () { keys.block = false; }, 0); },
    ...
    // Resize all rows.
    $(TblGridSplittedBagsId + ' tr').addClass('trclass');
}
