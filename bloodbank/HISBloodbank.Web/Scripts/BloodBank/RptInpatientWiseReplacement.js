...
$(document).ready(function () {
    ...
    c.SetTitle("Inpatient Wise Replacement");
    c.DefaultSettings();
...
function GenerateReport() {
    ...
    var Id = c.GetValue('#RegistrationNo');

    if (!c.GetICheck('#iChkPIN')) {
        Id = -1;
    }
    
    var dlink = hissystem.appsserver() + hissystem.appsname() + "/Areas/BloodBank/Reports/RptInpatientWiseReplacement/RptInpatientWiseReplacement.aspx";
...
function InitICheck() {
    $('#iChkPIN').iCheck({
        ...
        c.DisableSelect2('#select2PIN', !checked);
        c.DisableDateTimePicker('#dtFromDate', checked);
        c.DisableDateTimePicker('#dtToDate', checked);

        if (!checked) {
            c.SetSelect2('#select2PIN', '', '');
        }
...
function InitSelect2() {
    $('#select2PIN').select2({
        ...
        ajax: {
            quietMillis: 150,
            url: baseURL + 'Select2RptInpatient',
            dataType: 'jsonp',
            cache: false,
        ...
        c.SetValue('#RegistrationNo', list[2]);
    });
...
function DefaultValues() {
    ...
    //c.SetSelect2('#select2OutsideHospital', -1, '< ALL HOSPITALS >');
