...
function InitSelect2() {
    ...
    $("#select2Type").select2({
        containerCssClass: "RequiredField",
        data: [{ id: 1, text: 'Blood' }, { id: 2, text: 'Component' }, { id: 3, text: 'Procedure' }],
...
function DefaultEmpty() {
    ...
    c.SetValue('#txtAbbreviation', '');
    c.SetValue('#txtReplacement', '');

    c.ClearSelect2('#select2Type');
    c.ClearSelect2('#select2Department');

    c.iCheckDisable('#iChkWholeBody', false);
...
function HandleEnableEntries() {
    ...
    if (Action == 0 || Action == 3) { // view or delete
        ...
        c.Disable('#txtAbbreviation', true);
        c.Disable('#txtReplacement', true);

        c.iCheckDisable('#iChkWholeBody', true)

        c.DisableSelect2('#select2Type', true);
        ...
        c.Disable('#txtAbbreviation', false);
        c.Disable('#txtReplacement', false);

        c.iCheckDisable('#iChkWholeBody', false)

        c.DisableSelect2('#select2Type', false);
...
function View(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            c.SetSelect2('#select2Type', data.type, data.TypeName);
            ...
            c.SetValue('#txtAbbreviation', data.TempName);
            c.SetValue('#txtReplacement', data.ReplacementCount);
            c.SetValue('#txtArabicName', data.Arabicname);
            c.SetValue('#txtArabicCode', data.Arabiccode);
            c.iCheckSet('#iChkWholeBody', data.DefaultType);
...
function ShowListColumns() {
    ...
    { targets: [4], data: "Arabiccode", className: '', visible: true, searchable: true, width: "0%" },
    { targets: [5], data: "Arabicname", className: '', visible: true, searchable: true, width: "0%" },
    { targets: [6], data: "TypeName", className: '', visible: true, searchable: true, width: "0%" },
    ];
    return cols;
}
