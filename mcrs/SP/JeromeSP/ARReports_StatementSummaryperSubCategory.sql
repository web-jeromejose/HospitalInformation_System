...
-- =============================================
alter PROCEDURE [MCRS].[ARReports_StatementSummaryperSubCategory]
(
...
if(@IsYearly = '1')
	Begin
							...
							--  declare @CategoryListIds varchar(Max)	=  '[70]'
							...
							SELECT @rowCount =1; -- Count(*) FROM   #temptblVisitIds
							SET @counter = 1;
...
Begin
								...
								--DECLARE @enddate2 DATETIME set @enddate2 = dateadd(day,+1,dateadd(day,-1,dateadd(month,datediff(month,-1,CONVERT(VARCHAR,@endDate)),0)))

							 

										WHILE @Startdate2 < dateadd(day,+1,dateadd(day,-1,dateadd(month,datediff(month,-1,CONVERT(VARCHAR,@endDate)),0)))
										BEGIN
											 
											 -------------START
																DECLARE @stDateMonth DATETIME set @stDateMonth =  dateadd(month,datediff(month,0,@Startdate2),0) 
																DECLARE @enDateMonth DATETIME set @enDateMonth = dateadd(day,+1,dateadd(day,-1,dateadd(month,datediff(month,-1,@Startdate2),0)))
																DECLARE @branchMonth VARCHAR(100), @addressMonth VARCHAR(100);
																SET @branchMonth = (SELECT TOP 1 Name + ' - ' + City FROM OrganisationDetails)   
																SET @addressMonth = (SELECT TOP 1 Address1 + ' - ' + City FROM OrganisationDetails)
																	...
																	INSERT INTO #TSummary  
																		...
																		INNER JOIN 
																				...
																				WHERE 
																						A1.CategoryID = @Category 
																					AND A1.Posted > 1 
																					AND A1.BillDateTime >= @stDateMonth AND A1.BillDateTime <= @enDateMonth 
																		...
																		WHERE 
																				A.CategoryID = @Category  
																			AND A.Posted <> 0 
																			AND A.BillDateTime >= @stDateMonth AND A.BillDateTime < @enDateMonth 
																			AND(@companyId = 0 OR B.ID  = @companyId)
																			--AND (@gradeId = 0 OR A.GradeID = @gradeId)
																		GROUP BY B.Code, B.Name, C.OPCount,d.name,A.BillDateTime     
																		...
																		*/
																		...
																		(              
																			...
																			WHERE 
																				...
																				AND d.ID = b.BILLTYPE 
																				AND b.IsInvoiced = 1 
																				AND b.InvoiceDatetime >= @stDateMonth AND b.InvoiceDatetime < @enDateMonth 
																				AND b.CategoryID = @Category 
																				AND a.ServiceID<>14 
																				--AND(@gradeId = 0 OR b.GradeID = @gradeId)
																			GROUP BY 
																		...
																		(
																			...
																			(
																				...
																				INNER JOIN ARIPBill B11  (nolock)
																					ON A11.BillNo = B11.BillNo 
																				WHERE 
																						InvoiceDateTime >= @stDateMonth AND InvoiceDateTime < @enDateMonth 
																					AND B11.CategoryID = @Category
																					AND A11.ServiceID <> 14 
																					AND B11.IsInvoiced=1 
																				--	AND (@gradeId = 0 OR B11.GradeID = @gradeId)
																		...
																		--			, SUM(IPCount+OPCount) AS TCount
																		--			,ROW_NUMBER() OVER(ORDER BY SubCategory) slno
																		--			,DATENAME(month, @StartDTCounter) as datemonthname
																		--		FROM #TSummary 
																		--		GROUP BY CCode, CompanyName ,SubCategory   
																		--		ORDER BY SubCategory            
																		--	END             
																		...
																		--			--, SUM(IPGross+OPGross) AS TGross
																		--			--, SUM(ROUND(IPGross-IPDiscount-IPDeductable,2))+SUM(OPGross-OPDiscount-OPDeductable) AS TNet
																		--			--,UPPER(@branchMonth) branch
																		--			--,@addressMonth address 
																		--			,ROW_NUMBER() OVER(ORDER BY SubCategory) slno   
																		--			,DATENAME(month, @StartDTCounter) as datemonthname 
																		...
																		insert into #TTotalSummaryOPIPNET(SubCategory,CCode,CompanyName,IPCount,OPCount,TCount,slno,datemonthname)
																				SELECT 
																					...
																					, SUM(IPCount+OPCount) AS TCount
																					,ROW_NUMBER() OVER(ORDER BY SubCategory) slno
																					,DATENAME(month, @Startdate2) as datemonthname
									...
									--	delete from #TTotalSummaryOPIPNET  where CompanyName IS NULL
										...
										DECLARE @tempCompanyIdMonth AS TABLE( CompanyId int identity,CompanyName Varchar(200))
										insert into @tempCompanyIdMonth
										select distinct(CompanyName) from #TTotalSummaryOPIPNET
										
										 
									
										DECLARE @rcounter1 AS INT
										DECLARE @MonthlyCountermonthly AS INT
										DECLARE @visitCatId AS BIGINT
		 
										SELECT @rcounter1 =  count(*) from @tempCompanyIdMonth
										SET @MonthlyCountermonthly = 1;

										
				WHILE (@MonthlyCountermonthly <=  @rcounter1)
					BEGIN

						insert into @tempAllMonthly (Company,Id,Jan_tcount,Jan_ipcount,Jan_opcount )
						select top 1 a.CompanyName, @MonthlyCountermonthly as Id,a.Tcount,a.IPCount, a.OPCount  from #TTotalSummaryOPIPNET a 
						where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'January'
						 
			
						update @tempAllMonthly 
						SET Feb_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'February' )
						,Feb_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'February' )
						,Feb_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'February' )
						 where Id = @MonthlyCountermonthly;

						 update @tempAllMonthly 
						SET Mar_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'March' )
						,Mar_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'March' )
						,Mar_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'March' )
						 where Id = @MonthlyCountermonthly;



						 update @tempAllMonthly 
						SET Apr_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'April' )
						,Apr_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'April' )
						,Apr_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'April' )
						 where Id = @MonthlyCountermonthly;

						 update @tempAllMonthly 
						SET May_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'May' )
						,May_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'May' )
						,May_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'May' )
						 where Id = @MonthlyCountermonthly;

						 update @tempAllMonthly 
						SET Jun_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'June' )
						,Jun_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'June' )
						,Jun_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'June' )
						 where Id = @MonthlyCountermonthly;

						 update @tempAllMonthly 
						SET Jul_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'July' )
						,Jul_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'July' )
						,Jul_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'July' )
						 where Id = @MonthlyCountermonthly;

						 update @tempAllMonthly 
						SET Aug_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'August' )
						,Aug_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'August' )
						,Aug_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'August' )
						 where Id = @MonthlyCountermonthly;

						 update @tempAllMonthly 
						SET Sep_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'September' )
						,Sep_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'September' )
						,Sep_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'September' )
						 where Id = @MonthlyCountermonthly;

						 update @tempAllMonthly 
						SET Oct_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'October' )
						,Oct_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'October' )
						,Oct_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'October' )
						 where Id = @MonthlyCountermonthly;

						 update @tempAllMonthly 
						SET Nov_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'November' )
						,Nov_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'November' )
						,Nov_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'November' )
						 where Id = @MonthlyCountermonthly;

						 update @tempAllMonthly 
						SET Dec_ipcount = (select Top 1 IPCount from #TTotalSummaryOPIPNET a  where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname= 'December' )
						,Dec_opcount  = (select Top 1 OPCount  from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'December' )
						,Dec_tcount = (select Top 1 Tcount   from #TTotalSummaryOPIPNET a where a.CompanyName =  (select Top 1 CompanyName from @tempCompanyIdMonth where CompanyId =  @MonthlyCountermonthly) and a.datemonthname = 'December' )
						 where Id = @MonthlyCountermonthly;

				SET @MonthlyCountermonthly = @MonthlyCountermonthly+1;
				END
