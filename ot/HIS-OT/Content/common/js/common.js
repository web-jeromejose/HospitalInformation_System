...
var dateTimeFormatOnPostDateJsCompare = "MM/dd/yyyy HH:mm:ss";
...
Common.prototype = {
    ...
    GetDateTimePickerActual: function (id) {
        try {
    ...
    AddMinutes: function (date, minutes) {
        ...
        var fin = result.toString('dd-MMM-yyyy HH:mm');
        return fin;
    },
    ...
    ClearTable: function (selector) {
        $(selector + " > tbody > tr").empty();
    },
    ...
    ClearAlliCheckCheckbox: function () {
        $("input:checkbox").iCheck("uncheck");
    },
    ClearAlliCheckRadio: function () {
    ...
    Select2ClearAll: function (id) {
        $("input[type='text']").select2('val', '')
    },
    InputSelect2ClearAll: function () {
        $("input").select2('data', { id: "", text: "" });
    },
    ...
    GetDateTimePickerDateTimeCompare: function (id) {
        try {
            var a = $(id).find("input").val();
            if (a.trim().length == 0) return "";
            var output = new Date();
            output = Date.parse(a);
            var formatted = output.toString(dateTimeFormatOnPostDateJsCompare);
            return formatted;
        } catch (err) {
            return "";
        }
    ...
    DTFormat: function(id, CallBack) {
    CallBack(moment(id).format('DD MMM YYYY hh:mm:ss A'));
},
    DTFormatValue: function(inputDate, inputTime, CallBack) {
    var value = inputDate + " " + inputTime;
    CallBack(moment(value).format('DD MMM YYYY hh:mm:ss A'));
},
    DTFormatShort: function(id, CallBack) {
    CallBack(moment(id).format('DD MMM YYYY'));
},
    DTFormatShortWithDelimiter: function(id, CallBack) {
    CallBack(moment(id).format('DD/MMM/YYYY'));
},
    ...
    MessageNotification : function(msg, module,type = null) {
            toastr.options = {
                "closeButton": true,
                "debug": true,
            ...
            if (type == 'error') { toastr.info(msg, module);
    ...
    MessageBoxErr: function (Title, Message      ) {
        ...
        swal({
            html: true, type: "error", title: Title, text: Message
        });  ;
        //bootbox.dialog({
        //    message: Message,
        //    title: Title,
    ...
    ,Sel2Client: function (input, data, CallBack) {
        $(input).select2({
            allowClear: true,
            placeholder: "All",
            ...
            formatResult: this.selectFormatName
        }).on('change', function () {
            CallBack($(this).select2('data'));
        });
        $(input).select2("enable", true);
    }
    ,selectFormatName: function (data) {
        var markup = "<table><tr>";
    ...
    , Select2Ajax: function (URL, input, required = false, CallBack) {
         $('#preloader').show();
        $(input).select2({
            containerCssClass: (required == true ? "RequiredField" : "")
            , placeholder: "Select Options"
            , minimumInputLength: 2
            , ajax: {
                url: baseURL + URL,
                data: function (query) { return { str : query }; },
                results: function (data2) {
                    var data = data2.list;
                    if (data === undefined) { data = data2 }
                    var results = []
        ...
        }).change(function (e) { CallBack($(this).select2('data')); });
        $(input).select2("enable", true);
        $('#preloader').hide();
    }
