+function($) {
    var c = new Common();
    var Tbldashboard;
    var Tbl_ItemList;
    var Tbl_newSelectedItem;

    $(document).ready(function () {
        ShowList();
        TableOnclick();
        GetList();
        keydown();
      


    });

    function TableOnclick() {

        $(document).on("click", ".modalEntry", function () {
            resetField_NewEntry();
        });
        $(document).on("click", "#tbl-newselected-items td", function () {
            if ($(this).hasClass("_rem")) {
                $(this).closest("tr").remove();
            }
        });

        $(document).on("click", "#btn-new-save", function () {
            NewSave();
        });
 
        $(document).on("click", "#TblDashboard td", function (e) {
            e.preventDefault();
            if (typeof $(this).closest("tr").find('td:eq(0)').html() != 'undefined') {
                //  var tr = $(this).closest('tr');
                // tr.toggleClass('selected');
                let rowdata = Tbldashboard.row($(this).parents('tr')).data();
                if (rowdata  == null) {
                    c.MessageBoxErr('Error...', 'Please refresh the page and try again.');
                    return false;
                }
                console.log(rowdata);
                View(rowdata.IPID, rowdata.ID);
            }
        });

        $(document).on("click", "#tbl-items-list td", function (e) {
            e.preventDefault();
            if (typeof $(this).closest("tr").find('td:eq(0)').html() != 'undefined') {
                var tr = $(this).closest('tr');
                let rowdata = Tbl_ItemList.row($(this).parents('tr')).data();

                if (!tr.hasClass('selected')) { tr.toggleClass('selected'); }
  
                if (rowdata == null) {
                    c.MessageBoxErr('Error...', 'Please refresh the page and try again.');
                    return false;
                }
              
                add_SelectedItem(rowdata);
             //   remove_SelectedItem(this);
            }
        });


        $(document).on("click", "#btn-can", function (e) {
            swal({
                title: "Are you sure to cancel this order?",
                text: "Your will not be able to recover all this items!",
                type: "warning",
                showCancelButton: true,
                confirmButtonClass: "btn-danger",
                confirmButtonText: "Yes, cancel it!",
                closeOnConfirm: false
            },
                function () {
                    let orderid = $('#btn-can').data('orderid');
                    if (orderid == "0") { c.MessageBoxErr("Error...", "No OrderId. Please refresh.."); return false; }

                    $.ajax({ // First Request
                        url: baseURL + "CancelOrder",
                        data: { OrderId: orderid },
                        type: 'post',
                       // contentType: 'application/json; charset=utf-8',
                      //  dataType: 'json',
                        success: function (data) {
                            swal({
                                title: "Congrats!",
                                text: "Your Order has been cancelled.",
                                type: "success",
                                showCancelButton: false,
                                confirmButtonClass: "btn-info",
                                confirmButtonText: "OKAY",
                                closeOnConfirm: true
                            },
                                function () {
                                    location.reload();
                                    return false;
                                });
                        },
                        error: function (xhr, desc, err) {
                            var errMsg = "Error in posting the data." + "<br>" + err + "<br>" + desc;
                            c.MessageBoxErr("Error...", errMsg);
                        }
                    });


                   
                });
        });


    }
   
 

  

   //--------------------------------------------VIEW DATA TABLE 
    function ShowList() {
        var Url = baseURL + "ShowList";
     

        $('#preloader').show();
        
        $.ajax({
            url: Url,
            data: {},
            type: 'get',
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            cache: false,
            beforeSend: function () {
            },
            success: function (data) {
                $('#preloader').hide();
                
               BindList(data.list);
             },
            error: function (xhr, desc, err) {
                $('#preloader').hide();
                var errMsg = err + "<br>" + desc;
                c.MessageBoxErr(errMsg);
            }
        });

    }
    function ShowListCallBack() {
        var rc = function (nRow, aData) {

           // let groupFeature = aData['FunctionName'];
            var $nRow = $(nRow);

            //if (aData.selected.length != 1) {
            //    $('#chkselected', nRow).prop('checked', aData.selected === 1);
            //    if (aData.selected === 1) {
            //        $('#chkselected', nRow).prop("disabled", "disabled");
            //    }
            //}

            //if (aData.selected.length != 1) {
            //    $('#chkselected', nRow).prop('checked', aData.selected === 1);
            //    if (aData.selected === 1) {
            //        $('#chkselected', nRow).prop("disabled", "disabled");
            //    }
            //}
            //var value = aData['Price'];
            //var $nRow = $(nRow);

            //CatORder
            //if (groupFeature !== "--") {
            //    $nRow.css({ "background-color": "#ffff5e" })
            //}
            ////RequestedTest
            //if (groupFeature == "--") {
            //    $nRow.css({ "background-color": "#CEFFC9" })
            //}
            //FoodOrderDetail
            //if (groupFeature == 3) {
            //    $nRow.css({ "background-color": "#fbf3b0" })
            //}
            ////BSPExecutionDetail
            //if (groupFeature == 4) {
            //    $nRow.css({ "background-color": "#fbb673" })
            //}
            ////ProceduresOrder
            //if (groupFeature == "--") {
            //    $nRow.css({ "background-color": "#ffa4f2" })
            //}

            //RHOrder
            //if (groupFeature == 6) {
            //    $nRow.css({ "background-color": "#15739E" })
            //}
            //ISSUED
            //else if (value == 1) {
            //    $nRow.css({ "background-color": "#d7ffea" })
            //}
            //    //Partial Issues
            //else if (value == 3) {
            //    $nRow.css({ "background-color": "#f5b044" })
            //}

        };
        return rc;

    }
    function BindList(data) {
         Tbldashboard = $('#TblDashboard').DataTable({
            cache: false,
            destroy: true,
            data: data,
            paging: true,
             ordering: false,
             "order": [[0, "asc"]],
            searching: true,
            info: true,
            pagingType: "full_numbers",
            // lengthMenu: [[200, 250, 500, -1], [200, 250, 500, "All"]],
            autoWidth: false,
            scrollX: "100%",
            // dom: '<"divtoolbar">Bfrtip',
         "dom": '  <"divtoolbar">rt <"bottom"flip B><"clear">',
          //  scrollCollapse: false,
            fnRowCallback: ShowListCallBack(),
            columns: [


                //VIEWING


                {
                    title: '#', className: ' ', visible: true, searchable: true, width: "3%", sortable: true,
                    "render": function (data, type, full, meta) {
                        var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                            '<span  style="font-size: 10px;"  >' + full.ID + '</span>' +
                            ' </div>';
                        return html;
                    }
                },
                {
                    title: 'OrderNo', className: ' ', visible: true, searchable: true, width: "3%", sortable: true,
                    "render": function (data, type, full, meta) {
                        var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                            '<span  style="font-size: 10px;"  >' + full.StationSlNo + '</span>' +
                            ' </div>';
                        return html;
                    }
                },
                {
                    title: 'PIN', className: ' ', visible: true, searchable: true, width: "5%", sortable: true,
                    "render": function (data, type, full, meta) {
                        var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                            '<span class="badge bg-light-blue" style="font-size: 12px;"  >' + full.Registrationno + '</span>' +
                            ' </div>';
                        return html;
                    }
                },
                {
                    title: 'Name', className: ' ', visible: true, searchable: true, width: "10%", sortable: true,
                    "render": function (data, type, full, meta) {
                        var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                            '<span  style="font-size: 10px;"  >' + full.PName + '</span>' +
                            ' </div>';
                        return html;
                    }
                },
                { 
                    title: 'Bed', className: ' ', visible: true, searchable: true, width: "1%", sortable: true,
                    "render": function (data, type, full, meta) {
                        var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                            '<span  class="label label-success" style="font-size: 10px;"  >' + full.Bed + '</span>' +
                            ' </div>';
                        return html;
                    }
                },
                {
                    title: 'Doctor', className: ' ', visible: true, searchable: true, width: "10%", sortable: true,
                    "render": function (data, type, full, meta) {
                        var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                            '<span  style="font-size: 8px;"  >' + full.Doctor + '</span>' +
                            ' </div>';
                        return html;
                    }
                },
              
                {
                    title: 'Operator', className: ' ', visible: true, searchable: true, width: "10%", sortable: true,
                    "render": function (data, type, full, meta) {
                        var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                            '<span  style="font-size: 10px;"  >' + full.Nurse + '</span>' +
                            ' </div>';
                        return html;
                    }
                },
                {
                    title: 'Date Time', className: ' ', visible: true, searchable: true, width: "10%", sortable: true,
                    "render": function (data, type, full, meta) {
                        var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                            '<span  style="font-size: 10px;"  >' + full.orderDateTime + '</span>' +
                            ' </div>';
                        return html;
                    }
                },



//                Bed: "206-A"
//Doctor: "MAHMOUD MOHAMED EL SOLAH"
//ID: 968739
//IPID: "193272"
//Nurse: "KEITH ALBERT L. REYES"
//PName: "WALED  SHERAGHA"
//Registrationno: "SA01.0001501033"
//StationSlNo: "O-565"
//orderDateTime: "2/23/2019 9:56:00 AM"




                //{
                //    title: 'Remove Function', className: 'noExport row ', visible: true, searchable: true, width: "10%", sortable: true,
                //    "render": function (data, type, full, meta) {
                //        if (full.FunctionName == "--") {
                //            return '';
                //        } else {

                //            var html =
                //                '<div class="form-group-sm"> ' +
                //                '<div class="col-sm-12"> ' +
                //                '<div class="input-group input-group-sm col-sm-12"> ' +
                //                '  <button type="button" class="btn btn-xs btn-danger col-sm-12 RmvFunction " ' +
                //                ' data-employname="' + full.EmpName + '"  ' +
                //                ' data-functionname="' + full.FunctionName + '"  ' +
                //                ' data-featurename="' + full.Name + '"  ' +
                //                ' data-featureid="' + full.FeatureId + '"  ' +
                //                ' data-userid="' + full.UserId + '"  ' +
                //                ' data-functionid="' + full.FunctionID + '"><i class="fa fa-times-circle"></i><small>Function</small></button> ' +
                //                ' </div>' +
                //                ' </div>' +
                //                ' </div>';
                //            return html;

                //        }

                //    }
                //},



            ]
            //,  buttons: [{ extend: 'excelHtml5', orientation: 'landscape', pageSize: 'a4', filename: 'ItAdmin-ModuleView', footer: true, message: 'messagef', messageBottom: 'test', messageTop: ' ', title: 'ITAdmin-ModuleView', text: '<button type="button" style="color: #fff;background-color: #5cb85c;border-color: #4cae4c;" class="btn-margin-left btn btn-xs pull-left"> <i class="glyphicon glyphicon-print"></i> Excel </button><br><br>' }]
            , buttons: [{
                extend: 'pdfHtml5', orientation: 'landscape', pageSize: 'A4', filename: 'NursingAdministration', footer: true, message: 'messagef'
                , messageBottom: ' ', messageTop: ' '
                , title: 'NursingAdministration REPORT '
                , text: '<button type="button" style="color: #fff;background-color: #5cb85c;border-color: #4cae4c;" class="btn-margin-left btn btn-xs pull-left"> <i class="glyphicon glyphicon-print"></i> PDF </button><br><br>'
                , customize: function (doc) {
                    //get date
                    var date = new Date();
                    Date.prototype.yyyymmdd = function () {
                        var yyyy = this.getFullYear().toString();
                        var mm = (this.getMonth() + 1).toString(); // getMonth() is zero-based
                        var dd = this.getDate().toString();
                        return yyyy + "/" + (mm[1] ? mm : "0" + mm[0]) + "/" + (dd[1] ? dd : "0" + dd[0]); // padding
                    };

                    var cols = [];
                    cols[0] = { text: date.yyyymmdd(), alignment: 'left', margin: [20] };
                    cols[1] = { text: ' ', alignment: 'right', margin: [0, 0, 20] };
                    var objFooter = {};
                    objFooter['columns'] = cols;
                    doc['footer'] = objFooter;
                    doc.defaultStyle.fontSize = 8; //2,3,4,etc
                    doc.styles.tableHeader.fontSize = 9; //2, 3, 4, etc

                    //doc.content.splice(1, 0,
                    // {
                    //     margin: [0, 0, 0, 12],
                    //     alignment: 'center',
                    //     image: 'data:image/png;base64,'
                    // }
                    //);
                }
                , exportOptions: {
                    columns: "thead th:not(.noExport)"
                }
            }
                //other button
                , {
                    extend: 'excelHtml5', orientation: 'landscape', pageSize: 'A4', filename: 'NursingAdministration_excel', footer: true, message: 'messagef'
                , messageBottom: ' ', messageTop: ' '
                    , title: 'NursingAdministration REPORT '
                , text: '<button type="button"  class="btn-margin-left btn btn-info btn-xs pull-left"> <i class="glyphicon glyphicon-print"></i> Excel </button><br><br>'
                , exportOptions: { columns: "thead th:not(.noExport)" }

            },
            ]


        });

        //TblGridList = $(TblGridListId).DataTable({
        //    cache: false,
        //    destroy: true,
        //    data: data,
        //    paging: true,
        //    ordering: false,
        //    searching: true,
        //    info: false,
        //    scrollY: 380,
        //    processing: false,
        //    autoWidth: false,
        //    dom: 'Rlfrtip',
        //    scrollCollapse: false,
        //    pageLength: 150,
        //    lengthChange: false,
        //    iDisplayLength: 25,
        //    fnRowCallback: ShowListRowCallBack(),
        //    columns: ShowListColumns()

        //});


    }

    function ajax(IPID, OrderId) {
        var patient_result;
        var patient_orderlist;
        let param1 = {
            ipid: IPID,
        };
        let param2 = {
            OrderId: OrderId,
        };

        $.ajax({ // First Request
            url: baseURL + "GetInpatient",
            data: param1,
            type: 'get',
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function (data) {
                x = data.list[0];               
                $("#prefixorderno").val(x.IPID);
                $("#txtpatient").val(x.PIN);
                $("#txtname").val(x.PatientName);
                $("#txtbed").val(x.Bed);
                $("#ward").val(x.StationName);
                $("#operator").val("");
                $("#txtage").val(x.Age);
                $("#txtsex").val(x.Sex);
                $("#txtcompany").val(x.CompanyName);
                $("#package").val(x.Package);
                $("#drugallergies").val(x.Allergy);
                $("#blood").val(x.BloodGroup);
                $("#txtdoctor").val(x.DocName);
            },
            error: function (xhr, desc, err) {
                var errMsg = "Error in posting the data." + "<br>" + err + "<br>" + desc;
                c.MessageBoxErr("Error...", errMsg);
            }
        });

        $.ajax({ //Seconds Request
            url: baseURL + "GetOrderList",
            data: param2,
            type: 'get',
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function (returnhtml) {
                $('#tbl-selected').DataTable({
                    cache: false,
                    destroy: true,
                    data: returnhtml.list,
                    paging: true,
                    ordering: false,
                    searching: true,
                    info: true,
                    pagingType: "full_numbers",
                    autoWidth: false,
                    scrollX: "100%", 
                    dom: 'Rlfrtip',                  
                    pageLength: 2,          
                    columns:
                        [{ data: "ServiceID", title:'ITEMID', className: '', visible: true, searchable: true, width: "1%" },
                            { data: "code", title: 'CODE', className: '', visible: true, searchable: true, width: "1%" },
                            { data: "name", title: 'NAME', className: '', visible: true, searchable: true, width: "1%" },
                            { data: "quantity", title: 'QTY', className: '', visible: true, searchable: true, width: "1%" },
    
                        ]
                });
            },
            error: function (xhr, desc, err) {
                var errMsg = "Error in posting the data." + "<br>" + err + "<br>" + desc;
                c.MessageBoxErr("Error...", errMsg);
            }
        });
    }
    function View(IPID, OrderId) {
       
        $("#btn-can").attr("data-orderid", OrderId);
        $('#preloader').show();
 
        $.when(ajax(IPID, OrderId)).done(function () { });
        jQuery.noConflict();

        $('#modal-showselected').modal('show');

        setTimeout(function () {
            $('#preloader').hide();
        }, 800);

      
        //$.ajax({
        //    url: Url,
        //    data: param,
        //    type: 'get',
        //    contentType: 'application/json; charset=utf-8',
        //    dataType: 'json',
        //    cache: false,
        //    beforeSend: function () {

        //    },
        //    success: function (x) {
        //        $('#preloader').hide();



     


        //        //if (list.length == 0) {
        //        //    Action = 1;
        //        //    HandleEnableButtons();
        //        //    HandleEnableEntries();
        //        //    RedrawGrid();
        //        //    return;
        //        //}

        //        //var data = list[0];

        //        //c.SetValue('#Id', data.ID);
        //        //c.SetValue('#Is24', data.Is24);

        //        //c.SetSelect2('#select2PIN', data.IPID, data.PIN);
        //        //c.SetSelect2('#select2Name', data.IPID, data.Name);
        //        //c.SetSelect2('#select2BedNo', data.BedID, data.BedNo);

        //        //c.SetValue('#txtOrderNo', data.OrderNo);
        //        //c.SetValue('#txtOperator', data.OperatorName);
        //        //c.SetValue('#txtDateTime', data.DateTime);
        //        ////c.SetValue('#txtDoctor', data.DoctorIdName);
        //        //c.SetValue('#txtAge', data.Age);
        //        //c.SetValue('#txtGender', data.Gender);
        //        //c.SetValue('#txtBloodGroup', data.BloodGroup);
        //        ////c.SetValue('#txtDoctorId', data.DoctorID);
        //        //c.SetSelect2('#select2Doctor', data.DoctorID, data.DoctorIdName);


        //        //BindSelectedProcedure(data.OtherProceduresOrderDetail);
        //        //HandleEnableButtons();
        //        //HandleEnableEntries();
        //        //RedrawGrid();


        //    },
        //    error: function (xhr, desc, err) {
        //        $('#preloader').hide();
        //        var errMsg = err + "<br>" + desc;
        //        c.MessageBoxErr(errMsg);
        //    }
        //});
    }


   //--------------------------------------------Create New DATA TABLE
    function resetField_NewEntry() {
       // BindItemList([]);
        
        $("#txtpatient_new").select2("val", "");
        $("#txtpatient_new").select2('open');
        $("#tbl-newselected-items tbody").html('<table></table>');
      
    }
    function add_SelectedItem(rowdata) {
      //  rowIndex = tblSelectedList.cell(cell).index().row;
        //checking if exisiting 
        var $ex = 0;
        $.each(Tbl_ItemList.rows().data(), function (i, re) {
            if (Tbl_ItemList.cell(i, 0).data() == rowdata.id  ) {
                $ex = 1;
            }
        });
        var ctr = 1;
        var b = true;
        $.each($("#tbl-newselected-items > tbody > tr"), function () {
            if ($(this).data('id') == rowdata.id) {
                b = false;
            }
            ctr = ctr + 1;
        });
        if (b) {
                var i = "<tr data-id='" + rowdata.id + "'><td>" + ctr + "</td>";
            i = i + "<td><b>" + rowdata.code + "</b>  <small><span class='label label-success' style='font-size: 9px;'>" + rowdata.name + "</span></small></td>";
                i = i + "<td><input class='form-control qty' type='number' title='Check the QTY!' value='1' id='qty_" + rowdata.id + "' /></td>";
                i = i + "<td class='_rem'><span class='btn btn-danger btn-sm'>Remove</span></td></tr>";

                $("#tbl-newselected-items tbody").append(i);
               
                $("input.qty").focus();
               
                //$("input.qty").keydown(function (event) {
                //    if (event.which == 13) {
                //        if ($(this).val() == "") {
                //             $(this).val("1");
                //        }
                  
                //    };
                //});
                //$("input.qty").mouseleave(function (event) {
                //    if ($(this).val() == "") {
                //        $(this).val("1");
                //    }
            
                //});
        } else {
            c.MessageBoxErr(" Nursing Administration ", rowdata.name + " is already selected!");         
        }
     
        console.log('add_SelectedItem');
        console.log(rowdata);

//START
   /*     var ctr = 1;
        var b = true;
        $.each($("#tbl-selected > tbody > tr"), function () {
            if ($(this).data('id') == x.id) {
                b = false;
            }
            ctr = ctr + 1;
        });
        if (b) {
            var i = "<tr data-id='" + x.id + "'><td>" + ctr + "</td>";
            i = i + "<td>" + x.text + "</td>";
            i = i + "<td><input class='form-control qty' type='text' id='qty_" + x.id + "' /></td>";
            i = i + "<td class='_rem'><span class='btn btn-danger btn-sm'>Remove</span></td></tr>";

            $("#tbl-selected tbody").append(i);
            $("input.qty").focus();
            $("input.qty").keydown(function (event) {
                if (event.which == 13) {
                    if ($(this).val() == "") {
                        $(this).val("1");
                    }
                    $("._select_item").select2('open');
                };
            });
            $("input.qty").mouseleave(function (event) {
                if ($(this).val() == "") {
                    $(this).val("1");
                }
                $("._select_item").select2('open');
            });
        }
        else {
            NotifyError(x.name + " is already selected!", _option);
            $("._select_item").select2('open');
        }
        */
//END

            //Tbl_newSelectedItem.row.add({
            //    id: rowdata.id,                
            //    qty: 1,
            //    code: rowdata.code,
            //    name: rowdata.name,
            //}).draw();
            //c.ReSequenceDataTable(tblSelectedList, 0);
            //InitSelectedPrice();
        //}
    } 
    function GetList() {
        console.log('GetList()');
      
        $("#txtpatient_new").select2({
            containerCssClass: "RequiredField"
            , placeholder: "Search"
            , minimumInputLength: 2
            , ajax: {
                url: baseURL + 'PatientList',
                dataType: 'json',
                delay: 250,
                data: function (query) {
                    return { Registrationno: query };
                },
                results: function (data) {

                    try {

                        var arr = $.map(data.list, function (o) {

                            return {
                                id: o["IPID"],
                                text: o["Registrationno"],
                                name: o["PName"],
                            };
                        })


                    } catch (e) {
                    }
                    return {
                        results: arr
                    };
                },
                cache: false

            }

        }).change(function (e) {
            $('#preloader').show();
            ShowNewListItem();
            var list = e.added;
            console.log(list);
           
            let param1 = {
                ipid: list.id,
            };

            $.ajax({ // First Request
                url: baseURL + "GetInpatient",
                data: param1,
                type: 'get',
                contentType: 'application/json; charset=utf-8',
                dataType: 'json',
                success: function (data) {
                    x = data.list[0];
                    $("#prefixorderno_new").val(x.IPID);
                    $("#txtpatient_new").val(x.PIN);
                    $("#txtname_new").val(x.PatientName);
                    $("#txtbed_new").val(x.Bed);
                    $("#ward_new").val(x.StationName);
                    $("#operator_new").val("");
                    $("#txtage_new").val(x.Age);
                    $("#txtsex_new").val(x.Sex);
                    $("#txtcompany_new").val(x.CompanyName);
                    $("#package_new").val(x.Package);
                    $("#drugallergies_new").val(x.Allergy);
                    $("#blood_new").val(x.BloodGroup);
                  //  $("#txtdoctor_new").val(x.DocName);
                    console.log(x);
                    c.SetSelect2('#txtdoctor_new', x.DoctorID, x.DocName);
                    $('div.dataTables_filter input').focus();
                    $("#txtdoctor_new").val();
                    $('#preloader').hide();
                },
                error: function (xhr, desc, err) {
                    $('#preloader').hide();
                    var errMsg = "Error in posting the data." + "<br>" + err + "<br>" + desc;
                    c.MessageBoxErr("Error...", errMsg);
                }
            });
           
        });



        c.Select2Ajax('DoctorList', "#txtdoctor_new",true, function (e) {
            console.log(e); 
        });
      
    }
    function ShowNewListItem() {

        $('#preloader').show();

        $.ajax({
            url: baseURL + 'BedsideItems',
            data: {},
            type: 'get',
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            cache: false,
            beforeSend: function () {
            },
            success: function (data) {
                $('#preloader').hide();

                BindItemList(data.list);
            },
            error: function (xhr, desc, err) {
                $('#preloader').hide();
                var errMsg = err + "<br>" + desc;
                c.MessageBoxErr(errMsg);
            }
        });
    }
    function BindItemList(data) {

        Tbl_ItemList = $('#tbl-items-list').DataTable({
            //   cache: false,
            destroy: true,
            data: data,
            //   paging: true,
            //   ordering: false,
            ///   searching: true,
            ///   info: false,
            ///   scrollY: 250,
            //  processing: false,
            autoWidth: false,
            dom: 'Rlfrtip',
            lengthMenu: [[5, 20, 50, -1], [5, 20, 50, "All"]],
            language : {
                "search": "Search Code/Name"
            },
            // scrollCollapse: false,
            //   pageLength: 150,
            //  lengthChange: false,
            //   iDisplayLength: 25,
            columns: [ 
                //VIEWING
                { data:'id',title: '#', className: ' ', visible: false, searchable: false, width: "1%", sortable: true },
                { data: 'code', title: 'Code', className: ' ', visible: true, searchable: true, width: "2%", sortable: true },
                { data: 'name', title: 'Name', className: ' ', visible: true, searchable: true, width: "3%", sortable: true },

                //{
                //    title: 'code', className: ' ', visible: true, searchable: true, width: "3%", sortable: true,
                //    "render": function (data, type, full, meta) {
                //        var html = '<div class="input-group input-group-sm col-sm-12"> ' +
                //            '<span  style="font-size: 10px;"  >' + full.name + '</span>' +
                //            ' </div>';
                //        return html;
                //    }
                //}, 
            ]
         });

        //TblGridList = $(TblGridListId).DataTable({
        //    cache: false,
        //    destroy: true,
        //    data: data,
        //    paging: true,
        //    ordering: false,
        //    searching: true,
        //    info: false,
        //    scrollY: 380,
        //    processing: false,
        //    autoWidth: false,
        //    dom: 'Rlfrtip',
        //    scrollCollapse: false,
        //    pageLength: 150,
        //    lengthChange: false,
        //    iDisplayLength: 25,
        //    fnRowCallback: ShowListRowCallBack(),
        //    columns: ShowListColumns()

        //});


    }
    function NewSave() {
        var new_data;
        new_data = []
        new_data = {}
      
        console.log(c.GetSelect2Id("#txtdoctor_new"));
        if ($('#txtpatient_new').val() == "") {
            c.MessageBoxErr("Nursing Administration", "Select Patient!!"); return false;
        }
        if ($('#txtdoctor_new').val() == "") {
            c.MessageBoxErr("Nursing Administration", "Select Doctor!!"); return false;
        }
        if ($('#tbl-newselected-items tr').length == 1) {
            c.MessageBoxErr("Nursing Administration", "Select Procedure!!"); return false;
        }
        
        
        new_data.IPID = c.GetSelect2Id("#txtpatient_new");
        new_data.DocId = $('#txtdoctor_new').val();
        new_data.selecteditem = [];
        $("#tbl-newselected-items > tbody > tr").each(function () {
            let itemid = $(this).data('id');
            qty = $('#qty_' + itemid).val();
            if (qty == "" || qty == "0" || qty < 0) {
                c.MessageBoxErr("Nursing Administration", "Check the QTY of the selected Item!!"); return false;
            }
            new_data.selecteditem.push({
                itemid: itemid
                , qty: qty
            });

        });
    
     

        $.ajax({
            cache: false,
            type: 'POST',
            url: baseURL + 'SaveNursingAdmin',
            data: JSON.stringify(new_data) ,
            contentType: "application/json",
            beforeSend: function () {
                $('#preloader').show();
            },
            success: function (data) {
                console.log(data);
                swal({ html: true, type: "success", title: data.Title, text: data.Message });;
                $('#preloader').hide();

            },
            error: function (xhr, desc, err) {
                $('#preloader').hide();
                var errMsg = "Error in posting the data." + "<br>" + err + "<br>" + desc;
                c.MessageBoxErr("Error...", errMsg);
            }
        });
 
    }
    function keydown() {

        $(document).bind('keydown', 'alt+n', function () {
            console.log('alt+n');
            $('.modalEntry').trigger('click');
        });

        $(document).bind('keydown', 'alt+s', function () {
            console.log('alt+s');
            if ($('#modalEntry').hasClass('in')) {
                $('#btn-new-save').trigger('click');
            }          
        });
        $(document).bind('keydown', 'alt+c', function () {
            console.log('alt+c'); 
            if ($('#modalEntry').hasClass('in')) { $('.btnclose').trigger('click'); }
            else if ($('#modal-showselected').hasClass('in')) { $('.btncloseview').trigger('click'); } 
        });
 
        $(document).bind('keydown', 'alt+r', function () {
            console.log('alt+r');
            $('#btnRefresh').trigger('click');
        });
   
    }




} (jQuery);